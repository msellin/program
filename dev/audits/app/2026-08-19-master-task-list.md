...
- [x] **PROG-3** — `keywords.ts` ported off the dead KV store, 2026-09-02. It had iterated `STORE.list({ prefix: "user-email:" })` since before the 2026-08-18 Postgres migration, so for two weeks the note-keyword review queue scanned a store nothing writes and returned nothing — indistinguishable, in the response, from a queue that genuinely had nothing in it. Now reads `user_states` from Postgres, paginated. Two improvements taken while in there: distinct users are counted by opaque `user_id` rather than the email embedded in the old KV key (an endpoint that returns no personal data should not assemble a list of addresses to get there), and the analysis moved to `src/lib/note-keywords.ts` where it is testable without a database — 11 cases, including Estonian tokenisation, the frequency floor, and the count-vs-distinct-users distinction.

- [x] **AUDIT-1** — **The legal pages describe a system that no longer exists, and nothing checks them.** Scope is an audit, not a one-line fix; raised 2026-09-01 while correcting the Sentry region claim.
  ...
  - **The email promise is kept, 2026-09-02.** `POST /api/admin/notify` sends a plain-text notice to every confirmed account. Admin-gated (404, not 403), `dry_run` defaults to TRUE, and a live send additionally requires `confirm: "SEND"` — the first real use of this will be a bad day and the interface should not be discovered then. Unconfirmed sign-ups are excluded: an address nobody proved they control is not a user, and mailing it on a breach would itself disclose that somebody tried to register with it. Recipients are BCC'd in batches of 100 with the sender in `to`; the response returns counts, never addresses. **Deliberately not automated** — nothing in the app calls it. The privacy page promises notification, not a system that mails people on its own. Resend added to the sub-processor list (EU region, eu-west-1 — same region as Supabase, unlike Sentry), with the note that addresses are sent only when a notice goes out rather than kept as a list there. API key is `sending_access` scoped to terav.fit, so it cannot manage Resend resources or send as the other domain on that account. Tests: `notify.test.ts`, 10 cases. **AUDIT-1's remaining scope is now only the periodic re-read of the legal pages against the shipped system** — every specific defect it was raised for is closed.
  ...
  - **Closed 2026-09-03 by building the missing mechanism.** Every AUDIT-1
    correction so far was found by a person reading the page — the KV
    sub-processor entry, the Frankfurt region, the blanket "no email
    attached". That is not a mechanism, and the headline was never that any
    one line was wrong. `src/lib/legal-disclosure.test.ts` now pins the
    disclosures that ARE derivable from the codebase and deliberately leaves
    alone the ones that are not: retention periods and DPA questions are legal
    judgements and stay the founder's.
    Three defects it found, all on the landing, none previously recorded:
    (a) **`landing/privacy` and `landing/terms` stamped "Last updated {new
    Date().getFullYear()}"** — a freshness claim computed from the render
    clock. Both pages re-declared themselves current on every build whether or
    not a word changed, and would have said 2027 next January with no edit. A
    page that cannot go stale cannot be *detected* as stale, which is AUDIT-1's
    own failure mode mechanised into the page. Now literal dates.
    (b) **The two privacy pages promised different things.** The app page says
    policy changes are "announced by email", which `POST /api/admin/notify`
    makes true; the landing said "announced in-app", which nothing backs.
    Corrected to match the mechanism that exists — a correction of a false
    statement, not a new promise.
    (c) **Fourteen stale `KV` comments across nine files** still described the
    live persistence path as Cloudflare KV. Harmless to the runtime and
    exactly what misled the last reader into the sub-processor error; the
    genuinely historical mentions ("the legacy KV sync", "post KV → Postgres
    migration") were left alone.
    Guards: no legal page may compute its own date; every processor the code
    demonstrably talks to (Supabase, Sentry, Resend) must be named on the
    privacy page; the sub-processor list may not name Cloudflare KV unless a
    KV adapter exists; the two privacy pages may not disagree on the
    announcement channel. One-directional on purpose — Paddle is disclosed
    ahead of the paid tier, and disclosing early is not a defect.
    Mutation-tested: restoring the computed date and fully undisclosing Sentry
    each fail.
...
- [x] **P2-32** — Icon discipline closed 2026-09-02, but **not by the codemod it proposed**. Reading the actual usage changed the answer twice. First, several of the counted stroke widths are not icons at all — Recharts series, Sparkline paths, BarVisualizer geometry — so the "6 weights in circulation" figure was partly data-viz. Second, what remained is a deliberate system rather than drift: 2 as lucide's default, 1.75 for quiet secondary icons, 3 for the 11px selection ticks that do not read lighter, and `active ? 2.25 : 1.75` in the bottom nav as a selected-state signal. A codemod would have flattened intent, and a global icon restyle is exactly the visual change that belongs in an audit sweep rather than a maintenance pass (see `feedback_no-ui-churn-between-audits`). Three genuine outliers fixed instead: a chevron at 2.25 among nineteen siblings at default, the codebase's only size-15 icon, and two `strokeWidth={2}` that merely restate the default. `icon-discipline.test.ts` pins the result — the allowed sizes and the four stroke weights each with a stated purpose — so the set can shrink but not grow. Mutation-tested: an off-scale size, an undocumented weight and a redundant default each fail the suite.
