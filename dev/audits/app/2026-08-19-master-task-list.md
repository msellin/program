...
- [ ] **BUG-4** — O10c tier-recommendation engine bug for `first-strict-pullup`. Program is missing from both `SELF_REPORT_TO_NUMERIC` and `SELF_REPORT_TO_TEST_VAR` proxy tables in `intake-tier.ts:258-327`. Intake answers cannot bind to `strict_pullup_max_reps` / `dead_hang_max_seconds` → both variables default to 0 → `tier_a_hang` matches deterministically. Founder-reported: "3-5 strict reps" answer → Tier A ("No hang yet"). Fix: register `first-strict-pullup` in both proxy tables + add regression test. Also add conservative-defaults entry (`intake-tier.ts:385-391`). Source: `2026-08-19-founder-observations-queue.md` O10c investigation. Files: `next-app/src/lib/engine/intake-tier.ts:258-327,385-391`, `next-app/src/lib/engine/intake-tier.test.ts` (regression), `next-app/public/data/programs/first-strict-pullup.json` (reference). Size: M
- [ ] **BUG-5** — O12 DateNav Home icon layout-shifts the forward-arrow when day changes. `DateNav.tsx:49-58` conditionally renders the Home button without a reserved slot; forward-arrow jumps ~40 px. Week already has the fix pattern (`invisible pointer-events-none` reserved slot at `week/page.tsx:211,258`). ~3-line change. Source: `2026-08-19-founder-observations-queue.md` O12 + motion-perf assessment. Files: `next-app/src/components/workout/DateNav.tsx:49-58`. Size: S
- [ ] **BUG-6** — S-1 Progress Export report chip tap-target `min-h-[36px]` fails Apple 44 px. Source: `2026-08-19-founder-obs-mobile-ux.md` §S-1. Files: `next-app/src/app/progress/page.tsx:150-155`. Size: S
- [ ] **BUG-7** — HERITAGE classifier data-flow gap. Rowing + engine-builder both author the classifier + `retest_metrics_mid_block` correctly, but `HeritageClusterChip` reads `store.retest_readings` which is never dual-written from `runs[]` ingest. Schema gate lands, data flow gap open — chip renders "RESPONDING" on synthesized baselines but the two-baseline HERITAGE rule can't actually classify. **Blocks REV-1 caveat 1 + REV-5 blocker (d).** Source: S5 REV-1 + REV-5. Files: `next-app/src/lib/engine/*` (ingest), likely `useStore.ts` retest-readings write path. Size: M
- [ ] **BUG-8** — PerProgramAdherenceCard reports `0/25 done` despite N logged sessions. Root cause: `legacy-to-blocks.ts:75-86` — blocks materialize as `state: "planned"` and never flip to `done` when `log_entry_id` attaches. Every CSM (and likely every multi-track) user with logs but no explicit "Mark done" tap sees the 0-count. **Blocks REV-4 promotion + surfaces on rowing (REV-5).** Source: S5 REV-4 P0. Files: `next-app/src/lib/engine/legacy-to-blocks.ts:75-86` + live-hydrate hook on log writes. Size: M
- [ ] **BUG-9** — GraduationCard + `retest_due` proposal fire simultaneously on graduated users. `ProposalStack` lacks a graduation gate; users see "YOU FINISHED · 8 weeks logged" alongside the RETEST proposal. **Not engine-builder-specific; disqualifies app-level graduation orchestrator.** Source: S5 REV-1 caveat 3. Files: `next-app/src/app/page.tsx` (ProposalStack render) + one gate. Size: S
- [ ] **BUG-10** — YourPlanCard four-way contradiction on rowing users. Reveal card + phase header + END-OF-BLOCK RETEST WINDOW OPEN banner + YOU FINISHED graduation card all render simultaneously. Source: S5 REV-5 P0-1 + rowing delta §3. Files: rowing route surfaces on Today. Size: M
- [ ] **BUG-11** — Half-shipped features: (a) `SignalsStrip.tsx:154-159` pushes `csm-amber-week` signal but expanded-body switch (:257-336) has no matching render branch → "+1 more" placeholder shown; (b) `csm.json:541` promises "≥3 amber days → drop 4×4 next week" but `schedule.ts` never consumes `derived_state === "amber"` — authored hook is phantom. **Blocks REV-4 promotion.** Source: S5 REV-4 P1s. Files: `next-app/src/components/SignalsStrip.tsx` + `next-app/src/lib/engine/schedule.ts`. Size: M

---

## Section B — P0 (highest ROI)

- [ ] **P0-8** — O8 palette-collision fix. Category ↔ status colors overload the same 5 tokens across 3-5 semantic axes. Status chips become **neutral-outlined pill + 6 px colored dot** (`border border-line-soft text-muted flex items-center gap-1.5` + `<span className="h-1.5 w-1.5 rounded-full bg-{semantic}" />`). Sweep ~15-20 sites: `programs/page.tsx:230-254`, `ProgramPreviewClient.tsx:194-210`, `HeritageClusterChip`, `PerProgramAdherenceCard`, Week per-track pill. Leave `bg-{semantic}/10 border-l-{semantic}` **banners** alone (one-per-view). Also neutralize the laterality spine (`ExerciseCard.tsx:125-131`). Source: `2026-08-19-founder-obs-visual-craft.md` O8. Files: `next-app/src/app/programs/page.tsx`, `next-app/src/app/programs/[slug]/ProgramPreviewClient.tsx`, `next-app/src/app/week/page.tsx`, `next-app/src/components/workout/ExerciseCard.tsx`, others via grep `bg-.*\/20 text-`. Size: L (~2h)
- [ ] **P0-9** — O5a hide PROVISIONAL programs from public catalog. Three programs render "PROVISIONAL" chips with no legend entry (`persona-strength/text/06-programs.txt:48,56,86`). Copy-clarity + design-lead concurrently recommend HIDE (not legend) — filter alongside `personal:true` in `programs/page.tsx`. Design-lead adds: **rename token to DRAFT** in the schema when hidden. `engine-builder-block-2`, `first-strict-pullup`, `muscle-up` disappear from the catalog. Source: `2026-08-19-founder-obs-copy-clarity.md` O5a + `2026-08-19-founder-obs-design-lead.md` Decision 3. Files: `next-app/public/data/programs/manifest.json` (add `status: "DRAFT"` filter), `next-app/src/app/programs/page.tsx` (extend `personal:true` filter). Size: S
- [ ] **P0-10** — O10c placement-page "How this was picked" disclosure. Current `Based on: {formatVars(inferred.vars)}` at `IntakeClient.tsx:539-544` is a raw variable dump. Ship the three-branch content model (Branch A confident, Branch B skipped-tests conservative-default, Branch C signals-conflict). Draft copy in `2026-08-19-founder-obs-copy-clarity.md`. Coordinate with BUG-4 fix — copy alone doesn't fix the engine bug, but stops the disclosure from making the mispick worse. Source: `2026-08-19-founder-obs-copy-clarity.md` O10c + `2026-08-19-founder-obs-design-lead.md` Decision 4. Files: `next-app/src/app/programs/[slug]/intake/IntakeClient.tsx:539-544`. Size: M
...
- [ ] **P1-71** — O14b exercise-card chevron on done/non-loadable cards reveals only "Add note" — weak WCAG 2.4.4 nudge. Fix: hide the chevron when `!isLoadable && !suggestion`, promote "Add note" to persistent secondary-action button next to Play/Details at `ExerciseCard.tsx:201-227`. When card IS `done`, swap chevron for `MessageSquare` icon. Source: `2026-08-19-founder-obs-accessibility.md` O14b + visual-craft. Files: `next-app/src/components/workout/ExerciseCard.tsx:71,194-198,201-227`. Size: S

### Visual craft (post-Batch-25 round + founder-obs round)

- [ ] **P1-72** — O1 wordmark alignment (app matches landing pattern). Bronze pip left + white wordmark; keep ReadinessDot as second slot on the right for state. `AppShell.tsx:147-154`. Landing-alignment audit also flags: converge here or move ReadinessDot out of the wordmark and into its own slot on Today (bundle with O3b brief). Source: `2026-08-19-founder-obs-visual-craft.md` O1 + `2026-08-19-founder-obs-landing-alignment.md` Finding 1. Files: `next-app/src/components/AppShell.tsx:147-154`. Size: S
- [ ] **P1-73** — O14a exercise-name `truncate` → `line-clamp-2 leading-snug text-[15px]` at `ExerciseCard.tsx:176`. "Active hang (scap-engaged)" and similar now wrap to 2 lines. Verified truncation in `persona-recover/mobile` ("High-bar back s...", "Block pull, mid-s..."). Source: `2026-08-19-founder-obs-visual-craft.md` O14a + mobile-ux + a11y. Files: `next-app/src/components/workout/ExerciseCard.tsx:176`. Size: S
...
- [ ] **P1-75** — O7 Programs catalog polish. Section headers `text-[15px]` → `text-[18px] font-semibold tracking-tight` (`programs/page.tsx:188`); category glyph muted + bump to `text-[22px] leading-none` (:187); inter-section spacing `space-y-5` → `space-y-8` (:115). Source: `2026-08-19-founder-obs-visual-craft.md` O7. Files: `next-app/src/app/programs/page.tsx:115,187-191`. Size: S

### Landing→app (post-Batch-25 round + founder-obs round)

- [ ] **P1-76** — Accept ≠ Apply verb drift on landing. Landing says "Accept or Ignore" twice; app uses context-specific verbs like "APPLY BUMP", "APPLY 5% LIGHTER". Fix the **landing** copy to say "apply … or ignore" (app verbs are more precise). Source: `2026-08-19-founder-obs-landing-alignment.md` Finding 2. Files: `landing/src/i18n/dictionaries/en.ts` (`how.step_03_body`, `beta.body`). Size: S
...
- [ ] **P1-81** — O2 drop Programs from top-nav strip. 0/5 peers (Runna/Whoop/Hevy/Pliability/GOWOD) keep Programs in top-nav. Remove from `AppShell.tsx:156-162`; add row to Profile → More section. Ships as part of the Batch 29 header collapse per design-lead. Source: `2026-08-19-founder-obs-mobile-ux.md` O2 + `2026-08-19-founder-obs-design-lead.md` Decision 1. Files: `next-app/src/components/AppShell.tsx:156-162`, `next-app/src/app/profile/page.tsx` (add row). Size: S
...
- [ ] **F8 · Batch 29 — IA restructure (header collapse + Today dashboard + Settings v1).** One shipping batch, not sequenced. Header collapses to TERAV wordmark + Settings icon only (kills Programs / Morning-check / ⋮ overflow). Today becomes DashboardBlock-composed dashboard (morning-check block, extras block, workout block per active plan). Session moves to a new `/session/[slug]` route so Today is a *dashboard*, not a session container. Events deleted. Evidence/Report/Extras relocate to Profile More. Settings v1 = sound + haptic (real toggles) + theme + language (placeholder rows). Source: `2026-08-19-founder-obs-design-lead.md` Decision 1. Estimate: 4-6h. Depends on: P1-72 (wordmark), P1-78 (ReadinessDot kill), P1-81 (Programs top-nav drop).
- [ ] **F9 · Batch 30 — DashboardBlock primitive.** New reusable component (eyebrow · title · lede · slot · optional CTA) applied across 5 surfaces: Today blocks, session workout blocks, exercise cards (O14 fixes), program preview info hierarchy, catalog category groupings. GOWOD category tiles REJECTED (5 programs too few; revisit at N ≥ 10). Bundles O15 HWPO block-visual + O7 catalog + O9 preview. Source: `2026-08-19-founder-obs-design-lead.md` Decision 2 + visual-craft O15/O9/O7. Estimate: 3-5h.
- [ ] **F10 · Batch 31 — Readiness ladder rename + `reviewed_by` schema + PROMOTIONS.** Rename PROVISIONAL → DRAFT (hide from catalog per P0-9). Add `reviewed_by`, `reviewed_at`, `status_history`, `review_evidence[]` schema fields. Add attribution row on program preview page. `VERIFIED` gates on `beta_completions >= 5` with subjective success ≥ 3/5. Source: `2026-08-19-founder-obs-design-lead.md` Decision 3 + copy-clarity O5b + S5 re-review round (6 general-purpose agents, 2026-08-19).
...
- [ ] **S6** Anterior-hip-rebuild REVIEWED policy. This program has zero citation structures — was correctly excluded from the 2026-08-17 citation review. Current ladder copy defines REVIEWED as "specialist audited citations against literature." **Decision (pick one):** (a) redefine REVIEWED for personal programs to mean "author's clinical record audited against clinical-context.json constraints" (needs landing-copy update); (b) block anterior-hip promotion until a physio actually reviews the clinical alignment; (c) exclude personal programs from the ladder entirely (never promote past REFERENCED). Source: S5 REV-6.
