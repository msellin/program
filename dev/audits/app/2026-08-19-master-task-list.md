...
- [ ] **PROG-3** — `functions/api/admin/keywords.ts` reads a store nothing writes. It iterates `STORE.list({ prefix: "user-email:" })` — Cloudflare KV, which stopped being the live path in the 2026-08-18 Postgres migration. So the note-keyword review queue, the whole point of which is a weekly founder review of unmatched phrasing, has been returning stale or empty results ever since and nothing said so. Found 2026-09-02 while writing the completions endpoint next to it. Same KV drift that put "Cloudflare KV" on the privacy page. Port to Postgres or delete the endpoint. Size: S
