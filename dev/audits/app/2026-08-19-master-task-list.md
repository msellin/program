...
- [ ] **BETA-1** Programs is not reachable from the bottom nav (Day / Plan / Record / Profile) — only via Profile rows, empty-state CTAs, and the GraduationCard. In an installed PWA there is no URL bar to fall back on. **Decision:** add a fifth tab, or a persistent Programs row on Profile? Source: `2026-08-26-findings-ledger.md` §J7. Size: S
- [ ] **BETA-2** `AuthGate` carves out `/programs` as public but `AppShell` gates first and does not, so program browsing is auth-only and the carve-out is dead code. Harmless for beta; matters if the landing deep-links a logged-out visitor into a preview. Source: §J6. Size: S
...
- [ ] **BUG-29** — Brief's rail row reads "6 sets · 93.5 kg" on a 5/3/1 day, but only set 1 is at the top weight; sets 2-6 are FSL at 65-75% TM. The summary overstates the working load and the real FSL weight is only discoverable by stepping into set 2. Founder spotted it 30 Aug. Size: XS
